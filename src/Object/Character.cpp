...
Character::Character(World* w) {
    ...
    MovementSpeed = 1;
    state = Character_State::Dropping;
    v0 = 1;
}
...
void Character::Update() {
    ...
    if (Have(Character_State::Jumping)) {
        //float time_since_jump = (SDL_GetTicks64() - Start_Jump_Time)/1000;
        //y = v0 * time_since_jump - 4.9 * time_since_jump * time_since_jump;
        //Position.y = y;
        Position += velocity * glm::vec3(0, 1.5, 0);
        // 超过最大高度 开始掉落
        if (Position.y > world->position.y + 0.3) {
            Position.y = world->position.y + 0.3;
            End(Character_State::Jumping);
            Start(Character_State::Dropping);
        }
    }
    else if (Have(Character_State::Dropping)) {
        Position += velocity * glm::vec3(0, -1.2, 0);
        // 掉落到地上后开始行走
        if (!world->OnGround(Position - tofeet)) {
            End(Character_State::Dropping);
            if (Have(Character_State::keepJump)) {
                Start(Character_State::Jumping);
                Start_Jump_Time = SDL_GetTicks64();
            }
            else Start(Character_State::Walking);
